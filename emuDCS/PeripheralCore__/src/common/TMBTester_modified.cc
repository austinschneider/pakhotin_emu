...
bool TMBTester::testU76chip(){
  ...
  unsigned short int power_up = 0x0000;
  ...
  for (int bit=6; bit>=0; bit--) {
    ...
    pattern_expect = pattern_expect;
    //
    //    (*MyOutput_) << "Write = " << std::hex << pattern_expect << std::endl;
    //
    tmb_->WriteRegister(vme_usr_jtag_adr,pattern_expect);
    //
    tmb_->tmb_set_boot_reg(after_power_up | TMB_HARD_RESET);    // assert hard reset
    tmb_->tmb_set_boot_reg(after_power_up);                     // de assert hard reset
    ...
    pattern_expect  &= 0x007F;                                  // mask out lowest 7 bits
    read_data       &= 0x007F;                                  // mask out lowest 7 bits
