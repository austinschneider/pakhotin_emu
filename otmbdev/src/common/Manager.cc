...
//#define XML_CONFIGURATION_FILE "/home/cscotmb/config/pc/pcrate37-config.xml"
...
namespace emu { namespace otmbdev {
    ...
    void Manager::firstUse()
    {
      ...
      putButtonsInGroup( "OTMB Firmware Testing" );
      //HardReset Tests
      addActionByTypename<TMBHardResetTest>(crate); 
      //Hot Channel Mask Registers Test

      //Walking Test / Fiber Test

      //Short Cosmic Run 

      putButtonsInGroup( "OTMB Utilities" );
       //Hard Reset FPGA

       //Clear OTMB Counters
       
       //Read Comparitor Badbits

       //Monitor GTX Registers
       addActionByTypename<TMBRegisters>(crate); 

       //Disable/Enable Fibers

       //Disable/Enable Cooper
       addActionByTypename<TMBDisableCopper>(crate);

       //Disable/Enable CLCT Inputs
       addActionByTypename<TMBEnableCLCTInput>(crate);
       addActionByTypename<TMBDisableCLCTInput>(crate); 

       //Read/Write Any OTMB Register
       addActionByTypename<TMBReadRegisters>(crate, this);
       addActionByTypename<TMBSetRegisters>(crate, this);



      putButtonsInGroup( "OTMB Development" );
       //DCFEB Fine Delay Scan

       //ALCT RX/TX scan
       addActionByTypename<SetAlctTofDelay>(crate);
       addActionByTypename<YPTest>(crate); // YP Test button
       addActionByTypename<SetAlctTxRx>(crate);
       addActionByTypename<SetAlctTxRxposneg>(crate);
       addActionByTypename<ConfigureAlct>(crate);  
       addActionByTypename<CrateConfigure>(crate); 
       addActionByTypename<ReadAlctTofDelay>(crate);  
    ...
    void Manager::defaultWebPage(xgi::Input *in, xgi::Output *out)
    {
      ...
      *out << HTMLDoctype(HTMLDoctype::eStrict)
           ...
           << endl
           << h1()
           << "OTMB Development and Testing Routines"
           << h1()
           << endl << endl;
	...
	*out << cgicc::form()
    ...
    template <typename T>
    void Manager::addActionByTypename(Crate * crate, emu::otmbdev::Manager* manager ) {
      if(!currentActionVector_) putButtonsInGroup(UNDEFINEDGROUP);
    ...
    template <typename T>
    void Manager::addCommonActionByTypename(Crate * crate, emu::otmbdev::Manager* manager ) {
